...
const Articoli = () => {
    ...
    return (
      <section className="w-full mt-16 sm:mt-24  md:mt-32 px-5 sm:px-10 flex flex-col items-center justify-center">
        ...
        {loading ? (
          <p className="mt-16 text-center text-gray-500">Caricamento articoli...</p>
        ) : articoli.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-16 mt-16">
            {articoli.slice(0,6).map((ithBlog,index) => {
              return(
          ...
          <p className="mt-16 text-center text-gray-500">Nessun articolo disponibile.</p>
        )}
      </section>
    );
  };
