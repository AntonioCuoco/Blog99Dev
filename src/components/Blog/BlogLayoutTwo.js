...
import { convertFromIsoStringToDate, isNil } from "@/src/utils/utils";
    ...
    <div className="group grid grid-cols-12 gap-4 items-center text-dark">
      ...
      <div className="col-span-12  lg:col-span-8 w-full flex flex-col ">
        ...
        <Link href={`/blog/${blog.titleArticle}`} className="inline-block my-1">
          <h3 className="font-semibold capitalize text-base sm:text-lg !text-[#0c0c0c] bg-gradient-to-r from-accent/50 dark:from-accentDark/50 to-accent/50 dark:to-accentDark/50 bg-[length:0px_6px]
                group-hover:bg-[length:100%_6px] bg-left-bottom bg-no-repeat transition-[background-size] duration-500 ">
            {blog.titleArticle}
          </h3>
        </Link>
        <Link href={`/blog/${blog.titleArticle}`} className="inline-block my-1">
          <p className="capitalize text-base sm:text-base">
            {blog.subTitle}
          </p>
        </Link>

        <span className="inline-block w-full capitalize !text-[#64748b] dark:text-light/50 !font-semibold  text-xs sm:text-base">
          {
